...
export function ShareCard({ selfSkill, selectedFork, proverb, setProverb, onRestart }: { selfSkill: SelfSkill; selectedFork: ForkPath; proverb: string; setProverb: (x: string) => void; onRestart: () => void }) {
  ...
  return (
    <section className="space-y-4">
      <div className="mx-auto max-w-xl rounded-3xl border border-white/20 bg-gradient-to-br from-deep to-night p-8 text-center shadow-glow">
        ...
        <p className="mt-2 text-mist">第三条路：{selectedFork.title}</p>
        <p className="mt-3 text-gold">未来自我箴言：{proverb}</p>
        <p className="mt-4 text-xs text-mist">不是预测命运，而是召唤可能性。</p>
      </div>
