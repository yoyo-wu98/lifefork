...
export const createSelfSkillSlice: LifeforkSlice<SelfSkillSlice> = (set, get) => ({
  ...
  createSkill: async () => {
    ...
    set({ isGenerating: true, step: "generating", lastGenerationSource: null });
    ...
    try {
      ...
      set({
        ...
        badge: apiResult?.meta.llmUsed
          ...
          : shouldUseCulturalApi
            ...
            : "个人分析已生成",
        isGenerating: false,
        lastGenerationSource: apiResult?.meta.llmUsed ? "ai" : "local",
        step: "self-skill",
      });
      ...
      // Never leave the user stuck on the generating screen.
      set({
        isGenerating: false,
        badge: "生成遇到问题，请重试",
      });
      get().requestConfirmation(
        {
          title: "生成失败",
          message: "生成过程中遇到意外错误。你的回答已保存在浏览器中，可以直接重试。",
          confirmLabel: "重试",
          tone: "danger",
        },
        () => void get().createSkill(),
      );
    }
  },
  ...
  tuneVoice: (note) => {
    ...
    const alreadyRecorded = selfSkill.voice.calibrationNotes.includes(note);
    ...
    const updated: SelfSkill = {
      ...
      voice: {
        ...selfSkill.voice,
        calibrationNotes,
        // Score only grows when a genuinely new preference is recorded —
        // clicking the same chip repeatedly no longer inflates it.
        closenessScore: alreadyRecorded
          ? selfSkill.voice.closenessScore
          : Math.min(96, selfSkill.voice.closenessScore + (note === "像我" ? 3 : 5)),
      },
    };
